...
#!/bin/bash
notify-send $SNAME "ClamAV is scanning..." && SVAR=$DATEP;/usr/bin/clamscan -l $HOME/.ClamAV/Logs/$SNAME-$SVARP $SETP $SCANDIR 2>/dev/null;DVAR=$TAILP;notify-send "Done, Infected Files:"$DVARP
EOT
;;
*)
cat << EOT > $PWD/Schedule/$SNAME.sh
